...
Within this section we will create different data plots to analyze the trands within our data and see what kinds of relationships exist within it. This will inform us to what might be good trands to test later with hypothesis testing and regression tests. 
...
The above graph is not very helpful as the data is clumped and there isn't a clear picture of what is going on. We can try to get a clearer picture of the relationship between time and suicides by summarizing our data per year. We can do this by first combining each age group oer country into one total number of suicides per year in each of our countries. Then we can take the average per year across all the countries to get a larger view of how suicides trends overall as time progresses.
...
However this doesn't look like a very strong correlation, as it has large variations, and we can attempt to find better predictors to discover more about the data, say looking at rates within different age groups rather than suicides over time. Since the age groups act more at categorical variables rather than a continum in this data set and we care about occurrances within each group we can utilize a bar graph rather than a scatter plot. This will allow us to identify differences in suicide rates between the age groups represented in the data.

```{r step1graph3, include=TRUE}

# We first group over age and summarize the graph to get solidified numbers about suicides 
# per age group. Then we fit it to a bar graph to see the discrepencies in occurrances 
...
The bar graph above shows that there is a clear discrepency between the different age groups and suicide rates. Where those within the age range of 35-54 seem to be much more at risk than the other age groups. We can use this data to help predict if a person is more at risk of suicide by assessing their age group's rate of suicide and using this as part of a prediction strategy.

Another aspect that should be taken into account is a person's gender. As within western cultures (and cultures in general) the role of a person can very based off their gender.
...
With the added coloring based off sex we can also see that (with the exception of the 5-14 age range) the on average the majority of each age group committing suicide appears to be men. This, at a glance, shows us that men are more at risk when it comes to suicide than women.
...
Lastly if we observe each of these we can see a slight upward trend in recent years so lets look at this graph but only looking in the 21st century years (post 2000).
...
Within null hypothesis testing we set a standard alpha level (also known as rejection threshold) of 0.05.  Meaning we want our p-value for the model to be smaller than our threshhold. Based off the above data we can see year is not a good predictor for suicide occurrances by itself.

To further stress this idea, lets look at a graph of the residuals against the fitted values for this model. This graph will show us how well the model predicts the data by adhearing to certain properties based off the assumptions of linear regression. We would expect a good model to have a residuals graph that is centered around 0 and has no obvious correlation between the x and y axis.
...
We can use the residuals to see what type of relationship year could have with them in order to more accurratley predict suicide rates. If a factor that is known to affect suicide chances has a trend within the residuals of the year only model then the factor mostl ikely has an interaction with year. However, if this trend is not found the factor should be used as an additional predictor rather than one that has an interaction with year.
...
The above graph now atleast adheres to one property for a good residuals graph, being centered around 0.  However the graph does show a clear relathionship between the fitted and redisual values. This could infer that there is another factor affecting our model that is not taken into account.

From our analysis in Part 2 we also know that sex is a strong differenciator when it comes to suicide. Specifically in Part 2 we showed men and women tend to have large differences in suicide rates even within the same age group, this would lead us to want to make a graph of sex against our residuals to see if sex as a predictor should be used in a relationship or its own predictor.
...
Since there does seem to be a slight trend between sex and our residual values, shown by how female and male trend upward from one to another, we should add sex as a relational predictor rather than its own independent factor. Lets see what the model looks like under this new addition.
...
We can further analyze our different models by preforming an anova test on each in order to analyze their abilities to reject the null and minimize the residual sum of squares.
